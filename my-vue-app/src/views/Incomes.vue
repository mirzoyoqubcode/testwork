<template>
  <div class="p-6 max-w-[1920px] mx-auto">
    <div class="text-center mb-8">
      <h1 class="text-3xl font-bold text-black mb-2">Incomes</h1>
      <p class="text-gray-600">
        View and analyze income data with comprehensive filtering
      </p>
    </div>
    ...
    <div v-else class="space-y-6">
      ...
      <!-- Table -->
      <div class="bg-white border border-gray-200 rounded-lg p-6">
        <h2 class="text-xl font-semibold text-black mb-4">Incomes Details</h2>
        <DataTable :data="incomesData" :columns="tableColumns" />
      </div>
      ...
      >
        <div class="text-sm text-gray-600 text-center">
          Showing {{ incomesData.length }} of {{ totalRecords }} records
        </div>
...
import { apiService, type IncomeItem } from "../services/api";
import DataChart from "../components/DataChart.vue";
import DataTable from "../components/DataTable.vue";

const incomesData = ref<IncomeItem[]>([]);
...
const tableColumns = [
  { key: "income_id", label: "Income ID", type: "number" as const },
  ...
  { key: "date_close", label: "Close Date", type: "date" as const },
];

const warehouseOptions = computed(() => {
  const warehouses = new Set(
    incomesData.value.map((item) => item.warehouse_name)
  );
  return Array.from(warehouses).sort();
});

const chartData = computed(() => {
  let filteredData = incomesData.value;
...
const loadData = async () => {
  ...
  try {
    ...
    error.value = "Failed to load incomes data. Please try again.";
...
const loadMore = async () => {
  ...
  try {
    ...
    const response = await apiService.getIncomes(params);
    incomesData.value = [...incomesData.value, ...response.data];
    currentPage.value = response.current_page;
    totalPages.value = response.last_page;
    totalRecords.value = response.total;
  } catch (err) {
